...
//			if (!noMouse)
	...
	void UpdateKeys()
	{
		...
		{
			if (keyState[SDLK_z] || keyState[SDLK_BACKSPACE] || keyState[SDLK_u] || keyState[SDLK_LALT]) 
			{
				Undo();
				return;
			}
		...
		if (isMap && !editMode)
		{
			...
			else if ((keyState[SDLK_RETURN] | keyState[SDLK_LCTRL]  | keyState[SDLK_KP5] | keyState[SDLK_SPACE] | keyState[SDLK_KP_ENTER]) & 2) 
			{
